class Paddle {
  ...
  Paddle(float _width, float _height){
    this(width/2, height-20, _width, _height);
  }
  ...
  boolean detectBall(WreckingBall ball){
    ...
    if((yLoc + 10 >= y - pHeight/1.65) && (yLoc <= y + pHeight/1.65) && (xLoc + 10 >= x - pWidth/1.7) && (xLoc - 10 <= x + pWidth/1.7)){
      float bounce = xLoc - x;
      if(bounce >= 0 && !ball.hitBack){
        ball.hitBack = true;
        ball.modXVel(abs(bounce)/10.0, 1);
      }else if(bounce < 0 && !ball.hitBack){
        ball.hitBack = true;
        ball.modXVel(abs(bounce)/10.0, 0);
      }
      ball.paddleInvertY();
      return true;
    }else if((xLoc + 10 >= x - pWidth/1.9) && (xLoc <= x) && (yLoc >= y - pHeight/1.9) && (yLoc <= y + pHeight/1.9)){
      ball.invertXVel();
    } else if ((xLoc - 10 <= x + pWidth/1.9) && (xLoc >= x) && (yLoc >= y-pHeight/1.9) && (yLoc <= y + pHeight/1.9)){
      ball.invertXVel();
    }
